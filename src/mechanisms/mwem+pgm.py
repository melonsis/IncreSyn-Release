...
def mwem_pgm(data, epsilon, delta=0.0, workload=None, rounds=None, maxsize_mb = 25, pgm_iters=100, noise='laplace'):
    ...
    for i in range(1, rounds+1):
        ...
        est = engine.estimate(measurements, total)
        cliques.append(ax)
    print('Saving model, cliques and measurements...')
    
    measurefile = open("../data/measurements",'w')
    measurefile.write(str(measurements))
    measurefile.close()

    cliquefile = open("../data/cliques",'w')
    cliquefile.write(str(cliques))
    cliquefile.close()

    est.save(est,"../data/models")
