...
class Factor:
    def __init__(self, domain, values):
        ...
        :param domain: the domain of the factorw
    ...
    def expand(self, domain):
        assert domain.contains(
            self.domain), 'expanded domain must contain current domain'
    ...
    def transpose(self, attrs):
        assert set(attrs) == set(
            self.domain.attrs), 'attrs must be same as domain attributes'
    ...
    def condition(self, evidence):
        ...
        slices = [evidence[a] if a in evidence else slice(
            None) for a in self.domain]
    ...
    def __sub__(self, other):
        ...
        other = Factor(other.domain, np.where(
            other.values == -np.inf, 0, -other.values))
        return self + other
